...
async function runMember4Migration() {
  ...
  const sports = ['Cricket','Football','Basketball','Volleyball','Badminton','Kabaddi','Table Tennis','Chess','Carrom','Athletics'];
  for (const name of sports) {
    await db.query('INSERT IGNORE INTO sports (name, description, status) VALUES (?, ?, \'active\')', [name, `${name} campus sport`]);
  }
